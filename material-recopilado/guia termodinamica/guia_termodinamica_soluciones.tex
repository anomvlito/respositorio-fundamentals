...
\noindent\fbox{%
    \parbox{\textwidth}{%
        ...
        \begin{itemize}
            ...
            \item "Special Case: Constant Volume" implica $W=0$.
        \end{itemize}
    }%
}
...
\textbf{Interpretación:}
\begin{itemize}
    \item Por cada 1 kW de potencia útil producida
    \item Se debe absorber 1.394 kW de la fuente caliente
    \item Se rechaza 0.4 kW al sumidero frío
    \item Eficiencia del 71.75\% es el máximo teórico
\end{itemize}
...
\textbf{Solución:}
La entropía es una \textbf{propiedad de estado}, por lo que su cambio depende únicamente del estado inicial y final, no del camino seguido.

\textbf{Para un gas ideal}, el cambio de entropía se puede expresar como:
$$ \Delta S = m c_v \ln\left(\frac{T_2}{T_1}\right) + m R \ln\left(\frac{V_2}{V_1}\right) $$

O alternativamente:
$$ \Delta S = m c_p \ln\left(\frac{T_2}{T_1}\right) - m R \ln\left(\frac{P_2}{P_1}\right) $$

\textbf{Análisis de las opciones:}
\begin{itemize}
    \item \textbf{a) Masa transferida}: NO. En un sistema cerrado no hay transferencia de masa.
    \item \textbf{b) Calor transferido}: NO directamente. $\Delta S$ es propiedad de estado, el calor depende del camino.
    \item \textbf{c) Cambio de temperatura}: PARCIALMENTE. $\Delta S$ depende de $\Delta T$ pero también de otras variables.
    \item \textbf{d) Cambio de presión y volumen}: SÍ. Como propiedad de estado, $\Delta S = S(P_2, V_2) - S(P_1, V_1)$.
\end{itemize}

\textbf{Respuesta correcta:} La entropía depende del cambio de \textbf{presión y volumen} (variables de estado).

Alternativamente puede expresarse en función de $(T, P)$, $(T, V)$, o cualquier par de propiedades intensivas independientes.

\noindent\fbox{%
    \parbox{\textwidth}{%
        \textbf{Nota Handbook FE:}
        \begin{itemize}
            \item \textbf{Thermodynamics (Pág. 143):} Entropía listada como propiedad de estado.
            \item Propiedades de estado: $P, V, T, U, H, S$ (independientes del camino).
            \item Propiedades de trayectoria: $Q, W$ (dependen del camino).
        \end{itemize}
    }%
}
...
\textbf{Solución:}
\textbf{Balance de entropía para sistema abierto en estado estacionario:}
$$ \dot{S}_{gen} = \dot{m}(s_2 - s_1) - \frac{\dot{Q}}{T_b} $$

Como el sistema es \textbf{adiabático}: $\dot{Q} = 0$
...
\textbf{Paso 1: Calcular densidad del aire usando ecuación de gas ideal.}

De $Pv = RT$ o $P = \rho RT$:
$$ \rho = \frac{P}{RT} = \frac{200,000 \text{ Pa}}{287 \text{ J/(kg·K)} \times 293.15 \text{ K}} $$
$$ \rho = \frac{200,000}{84,134.05} = 2.377 \text{ kg/m}^3 $$

\textbf{Paso 2: Calcular área de la sección transversal.}
$$ A = \frac{\pi D^2}{4} = \frac{\pi (0.28)^2}{4} = \frac{0.2464}{4} = 0.0616 \text{ m}^2 $$

\textbf{Paso 3: Calcular flujo másico.}
...
$$ \frac{\text{kg}}{\text{m}^3} \times \text{m}^2 \times \frac{\text{m}}{\text{s}} = \frac{\text{kg}}{\text{s}} $$

\noindent\fbox{%
    \parbox{\textwidth}{%
        \textbf{Nota Handbook FE:}
        \begin{itemize}
            \item \textbf{Ideal Gas (Pág. 145):} $Pv = RT$ or $P = \rho R T$.
            \item \textbf{Fluid Mechanics (Pág. 178):} Rate of flow $\dot{m} = \rho A v$.
            \item Asegurar consistencia de unidades: usar Pa, J/(kg·K), K.
        \end{itemize}
    }%
}
...
Sin embargo, la convención puede variar. Si la pregunta pide el trabajo "efectuado por la bomba" en valor absoluto:
$$ |w_p| = 3.10 \text{ kJ/kg} $$
...
\begin{itemize}
    ...
    \item El agua puede existir en estado "superenfriado" (líquido bajo 0°C) bajo condiciones especiales
\end{itemize}
...
\noindent\fbox{%
    \parbox{\textwidth}{%
        \textbf{Nota Handbook FE:}
        \begin{itemize}
            \item \textbf{Thermodynamics (Pág. 143):} Zeroth Law: "Two systems in thermal equilibrium with a third system are in thermal equilibrium with each other."
