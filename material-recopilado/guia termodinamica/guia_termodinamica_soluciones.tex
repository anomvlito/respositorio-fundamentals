...
\noindent\fbox{%
    \parbox{\textwidth}{%
        \textbf{Nota Handbook FE:}
        \begin{itemize}
            \item \textbf{First Law (Pág. 147):} $W_b = \int P \, dV$ (Reversible boundary work).
...
\textbf{Datos:}
\begin{itemize}
    \item $P_1 = 57.83 \text{ kPa}$ (salida condensador = entrada bomba, estado líquido saturado)
    \item $P_2 = 3.060 \text{ MPa} = 3060 \text{ kPa}$ (entrada caldera = salida bomba)
...
De tablas de vapor a las condiciones de entrada de la bomba ($P_1 = 57.83 \text{ kPa}$, $T_{sat} \approx 85^\circ C$):
$$ v_f \approx 0.001033 \text{ m}^3\text{/kg} $$

\textbf{Paso 2: Relacionar con el FE Handbook para trabajo de bombas.}

El trabajo de una bomba ideal puede derivarse de la Primera Ley. En el \textbf{FE Handbook (Pág. 148, Steady-Flow Systems)}, el balance de energía para un volumen de control, asumiendo cambios de energía cinética y potencial despreciables y proceso adiabático ($Q_{in} = 0$), se reduce a:
$$ h_{in} = h_{out} + w_{out} \implies w_{pump\_in} = h_{out} - h_{in} $$

Para evitar buscar entalpías de líquido comprimido en tablas, el \textbf{FE Handbook (Pág. 149, Turbines, Pumps, Compressors)} nos entrega una relación explícita \textbf{para bombas asumiendo fluido incompresible}:
$$ h_{es} - h_i = v_i(P_e - P_i) $$
Donde el subíndice $s$ indica condición isoentrópica (ideal), $e$ es salida ($P_2$) e $i$ es entrada ($P_1$).

Por lo tanto, el trabajo consumido específico ideal es netamente:
$$ w_p = v_1(P_2 - P_1) $$

\textbf{Paso 3: Calcular el trabajo.}

Sustituyendo los valores:
$$ w_p = 0.001033 \times (3060 - 57.83) $$
$$ w_p = 0.001033 \times 3002.17 = 3.101 \text{ kJ/kg} $$

Dado que la bomba \textbf{consume} energía para operar, su trabajo neto extraído de un volumen de control sería formalmente negativo ($w_p = -3.10 \text{ kJ/kg}$), pero como el enunciado pide el trabajo \textit{efectuado por la bomba} (consumido en valor absoluto):
$$ |w_p| = 3.10 \text{ kJ/kg} $$
...
\begin{itemize}
    ...
    \item En ciclo Rankine, la bomba consume energía: $w_{in} = 3.10 \text{ kJ/kg}$
\end{itemize}
...
\noindent\fbox{%
    \parbox{\textwidth}{%
        \textbf{Nota Handbook FE:}
        \begin{itemize}
            \item \textbf{Thermodynamics (Pág. 148):} Steady-Flow Systems balance (Primera Ley).
            \item \textbf{Thermodynamics (Pág. 149):} Turbines, Pumps, Compressors. Ecuación explícita para el cambio de entalpía ideal en bombas (y trabajo reversible): $h_{es} - h_i = v_i(P_e - P_i)$.
        \end{itemize}
    }%
}
...
\begin{center}
    \includegraphics[width=0.6\textwidth]{images/FIS1523-2019-1-P23.png}
\end{center}
...
\textbf{Solución:}
Existen dos formas de interpretar la evolución hacia el equilibrio:

\textbf{1. Interpretación Estadística (Microscópica):}
A nivel molecular, la entropía está definida por la relación de Boltzmann:
$$ S = k_B \ln \Omega $$
Donde $\Omega$ es el número de microestados posibles (maneras en que se pueden ordenar las partículas).
\begin{itemize}
    \item Un macroestado con mayor cantidad de microestados ($\Omega$) es, por estadística, \textbf{más probable} que ocurra aleatoriamente.
    \item El estado de \textbf{equilibrio termodinámico} es la configuración de partículas más probable; por consiguiente, concentra el mayor número de microestados ($\Omega$ máximo).
    \item Al ser $\Omega$ máximo en el equilibrio, la entropía $S$ asume su valor \textbf{máximo}.
\end{itemize}

\textbf{2. Interpretación Clásica (Macroscópica):}
El enfoque tradicional de la Segunda Ley analiza las transferencias irreversibles, estipulando que un sistema evoluciona espontáneamente hacia el desorden hasta estancarse en el equilibrio.

\textbf{Conclusión:}
Independiente del enfoque de estudio utilizado, mientras más se aproxima o más probable es el estado de equilibrio, la entropía adopta su valor \textbf{máximo}.

\noindent\fbox{%
    \parbox{\textwidth}{%
        \textbf{Nota Handbook FE:}
        \begin{itemize}
            \item \textbf{Second Law of Thermodynamics (Pág. 151):} El manual adopta el enfoque clásico (macroscópico) estipulando en el \textit{Increase of entropy principle: $\Delta S_{isolated} \ge 0$}. La desigualdad enuncia matemáticamente que la entropía del universo progresa irremediablemente hasta estabilizarse en un \textbf{máximo} cuando el sistema aislado cruza la barrera del \textbf{equilibrio}. Al ser una guía técnica orientada  a cálculos directos, el manual FE no menciona la ecuación probabilística de Boltzmann ($S = k_B \ln \Omega$), centrándose exclusivamente en la desigualdad de Clausius.
        \end{itemize}
    }%
}
...
\textbf{Enunciado:} En Gran Bretaña la escala de temperatura utilizada es la escala Rankine. Determine en dicha escala, el valor de la temperatura correspondiente al punto de congelación y de ebullición del agua, respectivamente.

\textbf{Solución:}
La escala Rankine ($^\circ\text{R}$) es la escala absoluta de temperatura equivalente a la escala Fahrenheit ($^\circ\text{F}$), de la misma manera que Kelvin (K) es la escala absoluta para Celsius ($^\circ\text{C}$). El tamaño del grado Rankine es idéntico al del grado Fahrenheit.

La fórmula de conversión desde Fahrenheit hacia Rankine consiste en sumar la temperatura del cero absoluto en Fahrenheit:
$$ T (^\circ\text{R}) = T (^\circ\text{F}) + 459,67 $$

Para responder la pregunta, evaluamos los puntos clave del agua a nivel del mar (1 atm):

\textbf{1. Punto de Congelación del Agua:}
En la escala Fahrenheit, el agua se congela a $32^\circ\text{F}$. Aplicando la conversión:
$$ T_{\text{congelación}} = 32 + 459,67 = 491,67 \approx 491,7^\circ\text{R} $$

\textbf{2. Punto de Ebullición del Agua:}
En la escala Fahrenheit, el agua ebulle a $212^\circ\text{F}$. Aplicando la conversión:
$$ T_{\text{ebullición}} = 212 + 459,67 = 671,67 \approx 671,7^\circ\text{R} $$

Por lo tanto, la respuesta es $491,7^\circ\text{R}$ y $671,7^\circ\text{R}$.

\noindent\fbox{%
    \parbox{\textwidth}{%
        \textbf{Nota Handbook FE:}
        \begin{itemize}
            \item \textbf{Units (Pág. 1):} La tabla de conversiones define explícitamente $T_{^\circ\text{R}} = T_{^\circ\text{F}} + 459,67$. Además, documenta de manera literal los estados base del agua: \textit{Water freezes at $32^\circ\text{F}$ ($491,67^\circ\text{R}$) and boils at $212^\circ\text{F}$ ($671,67^\circ\text{R}$)}.
        \end{itemize}
    }%
}
...
\noindent\fbox{%
    \parbox{\textwidth}{%
        \textbf{Nota Handbook FE:}
        \begin{itemize}
            \item \textbf{Mechanics of Materials (Pág. 131):} Thermal Deformation $\delta_t = \alpha L \Delta T$.
...
De las tablas de saturación a $P = 0.6$ MPa (Pág. 157):
\begin{itemize}
    \item $T_{sat} = 158.85^\circ C$
\end{itemize}

Como $T = 700^\circ C > T_{sat} = 158.85^\circ C$, el agua se encuentra en estado de \textbf{vapor sobrecalentado}.

\textbf{Paso 2: Consultar tablas de vapor sobrecalentado.}

En las tablas de vapor sobrecalentado (\textit{Superheated Water Tables}) del FE Handbook (Pág. 158), buscamos la presión $P = 0.60$ MPa.

Para dicha presión, observamos directamente en la tabla el valor a la temperatura requerida:
\begin{itemize}
    \item A $T = 700^\circ C$: $s = 8.5107$ kJ/(kg·K)
\end{itemize}

\textbf{Conclusión:}
El valor exacto proporcionado por la tabla es \textbf{8.5107 kJ/(kg·K)}.

\noindent\fbox{%
    \parbox{\textwidth}{%
        \textbf{Nota Handbook FE:}
        \begin{itemize}
            \item \textbf{Steam Tables (Pág. 158):} Para presiones comunes y temperaturas de hasta $1300^\circ C$, el manual ofrece valores directos sin necesidad de interpolar si la temperatura coincide con las entradas de la tabla (como en este caso para 700°C).
        \end{itemize}
    }%
}
...
\noindent\fbox{%
    \parbox{\textwidth}{%
        \textbf{Nota Handbook FE:}
        \begin{itemize}
            \item \textbf{Properties for Two-Phase Systems (Pág. 144):} El manual define la calidad ($x$) y entrega las relaciones para propiedades en la mezcla:
            \begin{itemize}
                \item $h = h_f + x \cdot h_{fg}$
                \item $u = u_f + x \cdot u_{fg}$
                \item $s = s_f + x \cdot s_{fg}$
                \item $v = v_f + x \cdot v_{fg}$
            \end{itemize}
            \item Estas fórmulas se encuentran bajo el encabezado \textit{"Properties for Two-Phase (vapor-liquid) Systems"}.
        \end{itemize}
    }%
}
...
\noindent\fbox{%
    \parbox{\textwidth}{%
        \textbf{Nota Handbook FE:}
        \begin{itemize}
            \item \textbf{Second Law (Pág. 151):} Isothermal, Reversible Process $\Delta s = q/T$. Para cambios de fase: $\Delta s = h_{fg}/T$.
        \end{itemize}
    }%
}
