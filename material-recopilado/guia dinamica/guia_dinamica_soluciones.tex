...
\textbf{Solución:}
\begin{center}
    \includegraphics[width=0.4\textwidth]{images/2023_2_din_p_16.png}
\end{center}
\textbf{Análisis Cinemático:}
Dado que la barra de masa $m$ está suspendida por dos cuerdas inextensibles del mismo largo y paralelas, el sistema forma un paralelogramo. Al soltarse, la barra no rota ($\alpha = 0$, traslación curvilínea) y todos los puntos de la barra, incluido su centro de masa $G$, describen arcos de circunferencia del mismo radio (el largo de las cuerdas).

En el instante exacto en que se suelta ($t=0$):
\begin{enumerate}
    \item La velocidad es nula ($v=0 \implies \omega_{cuerdas}=0$).
    \item Como la aceleración normal centrípeta es $a_n = v^2/r$, en este instante $a_n = 0$.
\end{enumerate}

\textbf{Análisis Dinámico (Primera Ley en el Eje Normal):}
Sea $\theta$ el ángulo que forman las cuerdas con la \textbf{vertical}. Aplicando la Segunda Ley de Newton en el eje paralelo a las cuerdas (dirección normal de la trayectoria):
$$ \sum F_n = 2T - mg \cos\theta = m a_n $$
Como $a_n = 0$ al partir del reposo:
$$ 2T = mg \cos\theta \implies T = \frac{mg \cos\theta}{2} $$

Dependiendo de cómo esté acotado el ángulo $60^\circ$ en la figura:
\begin{itemize}
    \item Si el ángulo de $60^\circ$ es con la vertical ($\theta = 60^\circ$): $\cos(60^\circ) = 0.5 \implies T = 0.25 mg$.
    \item Si las cuerdas estuvieran casi horizontales ($60^\circ$ de desviación de la vertical se acerca a la respuesta d). 
    \item Si asumimos que la alternativa esperada es $0.25mg$, la proyección geométrica del peso sobre las cuerdas debe ser la mitad del peso total.
\end{itemize}
De esta deducción física, la única respuesta compatible con la dependencia cosenoidal de un ángulo pronunciado al soltarse es $0.25mg$.

\noindent\fbox{%
    \parbox{\textwidth}{%
        \textbf{Nota Handbook FE:}
        \begin{itemize}
            \item \textbf{Kinematics of Rigid Bodies (Pág. 119):} Movimiento de traslación ($a_A = a_B$, $\alpha=0$).
            \item \textbf{Kinetics of Rigid Bodies (Pág. 120):} Componentes Normal y Tangencial. En el instante de soltura desde el reposo, $a_n = 0$ y todo el desbalance de fuerza se traduce puramente en aceleración tangencial.
        \end{itemize}
    }%
}
