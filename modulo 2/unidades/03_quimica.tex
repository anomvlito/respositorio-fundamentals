...
\begin{itemize}
    \item \begin{definicion}[title=Equilibrio Químico] \fehandbook{85}
Para $aA + bB \rightleftharpoons cC + dD$:
$$ K_{eq} = \frac{[C]^c [D]^d}{[A]^a [B]^b} $$
\begin{itemize}
    \item Si $Q < K$: Reacción va a la derecha ($\to$).
    \item Si $Q > K$: Reacción va a la izquierda ($\leftarrow$).
\end{itemize}
\end{definicion}
...
\begin{itemize}
    \item \begin{teorema}[title=Ácidos y Bases] \fehandbook{86}
$$ pH = -\log[H^+], \quad pOH = -\log[OH^-] $$
$$ pH + pOH = 14 \quad (\text{a } 25^\circ C) $$
Para ácidos débiles: $K_a = \frac{[H^+][A^-]}{[HA]}$.
\end{teorema}
...
\begin{teorema}[title=Ley de Gases Ideales] \fehandbook{145 (Thermodynamics)}
$$ PV = nRT $$
Condiciones estándar (STP): $T=0^\circ$C (273.15 K), $P=1$ atm.
\end{teorema}
