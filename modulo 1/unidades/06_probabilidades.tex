...
\begin{itemize}
    ...
    \item \textbf{Axiomas}: $P(\Omega)=1$, $P(A) \ge 0$, etc.
\end{itemize}
\end{definicion}
...
\begin{definicion}[title=Medidas Teóricas]
Sea $X$ una variable aleatoria:
\begin{itemize}
    \item \textbf{Esperanza (Media):} $E[X] = \mu$.
    \item \textbf{Varianza:} $Var(X) = E[(X-\mu)^2] = E[X^2] - (E[X])^2$.
    \item \textbf{Coef. de Variación:} $CV = \frac{\sigma}{|\mu|}$. (Adimensional, útil para comparar dispersión).
\end{itemize}
\end{definicion}
...
Binomial & $n, p$ & Conteo de éxitos en $n$ intentos independientes. \\
Geométrica & $p$ & Intentos hasta el primer éxito. \\
Poisson & $\lambda$ & Tasa de ocurrencia de eventos raros en intervalo. \\
Normal & $\mu, \sigma^2$ & Modelado de fenómenos naturales (T. Central Límite). \\
Exponencial & $\lambda$ & Tiempo de espera entre eventos Poisson. \\ \hline
\end{tabular}
\end{table}
...
\begin{teorema}[title=Teorema del Límite Central]
...
\begin{definicion}[title=Regresión Lineal Simple]
Modelo: $Y = \beta_0 + \beta_1 X + \epsilon$
\begin{itemize}
    \item Coeficiente de Determinación ($R^2$): % de variabilidad explicada por el modelo.
